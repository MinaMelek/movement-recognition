...
def manipulate(human, name='', vid='', frame=0, mode='test', directory='No'):
...
#                print(j , human[i].body_parts[j])
    ...
    for i in range(len(human)):
        if mode == 'train':
            all_data.append(str(name))
        else:
            all_data.append(str(name)+"_Person"+str(i+1)) # Appends all data together in shape of x1, y1, x2, y2
        all_data.append(vid)
